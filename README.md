# Zadanie rekrutacyjne

## Opis

Projekt jest podzielony na dwie części (fronted i backend).

Do uruchomienia projektu potrzeba wykonać polecenie w konsoli
``` ./start.sh ```.


Pamiętaj o zainstalowaniu zależności przed prierwszym uruchomieniem.

W folderze `backend` znajduje się prosta plikacja obsługująca requesty dla frontendu. Nie będziesz trzeba nic tam zmieniać.

Twoje zadanie będzie dotyczyć dopisania funkcjonalności do aplikacji znajdującej się w katalogu `frontend`.

## Zadanie

1. Twoim zadaniem jest dopisać brakujące funkcjonalności, tak aby aplikacja wyświetliła w tablicy wszystkich pracowników wraz z informacjami o ich zatrudnieniu

![img](./assets/employees_1.png)

Do pobrania tych informacji z backendu musisz wykonać dwa requesty na dwa endpointy:
- ```/employess```
- ```/contracts```

Pobierz dane z tych endpointów i napisz kod aplikacji tak, aby była ona zbliżona do tego co widać na screenshotcie powyżej

2. Do tablicy dodaj paginacje. 
Ustaw wielkość strony na 5 elementów.
![img](./assets/employees_3.png)

   
3. Kliknięcie w danego pracownika powinno przenieść użytkowanika aplikacji na osobną stronę wyświetlajacą dodatkowe informacje o pracowniku
Potrzebne endpointy
- ```/employess/:id```
- ```/contracts/:id```

![img](./assets/employee_details.png)


4. Sformatuj daty tak, aby wygodnie moża było je czytać.


5. Zadbaj o to, aby na czas ładowania danych pokazał się spinner
![img](./assets/spinner.gif)

----

Dodatkowy filmik z działania aplikacji:
![movie](./assets/movie.gif)



---- 

Na co zwracamy uwage:
1. Czysty kod, uporządkowana struktura
2. Czy napisany kod da się łatwo reużyć w innym miejscu 
3. BEM w stylowaniu 
4. Optymalne przetwarzanie danych
5. Możesz zaproponować swój design o ile będzie lepszy niż ten podstawowy :) 
